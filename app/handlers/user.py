...
async def email_handler(message: Message):
    if utils.checkEmail(message):
        db.updateEmail(message.from_user.id, message.text)
        await start_handler(message)
    else:
...
async def email_handler(message: Message):
    if utils.checkTel(message):
        db.updateTel(message.from_user.id, phonenumbers.format_number(phonenumbers.parse(message.text, "UA"),
